...
if rank == 0 :
    ...
    try :
        d0 = time.time()
        Pr, info = pt.ERA(I, R, None, O, iters, hardware = 'gpu', mask=mask, alpha=1e-10, dtype='double')
        d1 = time.time()
        print '\ntime (s):', (d1 - d0) 
    except Exception as e:
        print e
...
if rank == 0 :
    ...
    try :
        d0 = time.time()
        Or, Pr, info = pt.ERA(I, R, None, None, iters, hardware = 'gpu', method = 5, mask=mask, alpha=1e-10)
        d1 = time.time()
        print '\ntime (s):', (d1 - d0) 
    except Exception as e:
        print e
